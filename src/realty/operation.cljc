(ns realty.operation
  "OperationActor -- one realty operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (Realtor-LLM) is
  sealed into a single node (:advise); its proposal is ALWAYS routed
  through the RealtorGovernor (:govern) and the rollout phase gate
  ...
  One graph run = one realty operation (intake -> advise -> govern ->
  ...
  decision to a human operator (the licensed real-estate agent / closing
  attorney). The approver resumes with `{:approval {:status :approved}}`
  (or :rejected). `:closing/submit` ALWAYS reaches this node when the
  governor is clean -- see `realty.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [realty.realtorllm :as realtorllm]
            [realty.governor :as governor]
...
(defn build
  ...
  `realty.store/Store`).
  opts:
    :advisor      -- a `realty.realtorllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (realtorllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Realtor-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (realtorllm/-advise advisor store request)]
            {:proposal p :audit [(realtorllm/trace request p)]})))

      ;; RealtorGovernor -- independent censor (separate system than the LLM).
      (g/add-node :govern
